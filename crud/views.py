...
class CreateSubcategory(APIView):
    ...
    def post(self, request):
        ...
        if category:
            data = CategorySub.objects.create(category=category, subcategory_name=subcategory_name)
...
class EditDeleteSubcategory(APIView):
    ...
    def put(self, request, id):
        ...
        subcategory_name = request.data['subcategory_name']
        try:
            data = CategorySub.objects.get(id=id)
            try:
                CategoryManagement.objects.get(id=category_id).exists()
                data.category = category
                data.subcategory_name = subcategory_name
                data.save()
                return Response({"status":"updated successfully"})
            except:
                return Response({"status":"invalid category"})
        except:
            return Response({"status":"subcategory does not exist"})
    def delete(self, request, id):
        id = id
        try:
            subcategory = CategorySub.objects.get(id=id)
            subcategory.delete()
            return Response({"status":"deleted successfully"})
        except:
            return Response({"status":"subcategory does not exist"})
...
class CreateProduct(APIView):
    ...
    def post(self, request):
        ...
        try:
            category = CategoryManagement.objects.get(id=category_id)
            try:
                subcategory = CategorySub.objects.get(id=subcategory_id)
                data = Products.objects.create(product_name=product_name, price=price, description=description, count_of_stock=count_of_stock, category=category, subcategory=subcategory)
